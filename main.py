...
import logging
import os
from fastapi import FastAPI, UploadFile, File, HTTPException, status, Request, Query, Body
...
# Add CORS middleware to allow frontend JavaScript to connect to the backend from any origin
app.add_middleware(
    CORSMiddleware,
...
async def log_requests(request: Request, call_next):
    ...
    """
    client_ip = request.client.host if request.client else "unknown" # Get the client's IP address
...
async def query_table_data(
    ...
    filters_string: Optional[str] = Query(None, alias="filters", description="Comma-separated key=value pairs for filtering (e.g., category=water,price=100)"), # Added for Swagger UI
):
    """
    Queries data from the specified table with optional filters, limit, and offset.
    Filters can be applied as key=value pairs in the 'filters' query parameter (e.g., category=water,price=100)
    or as individual query parameters (e.g., /data/mytable/query?category=water&price=100).
    
    - **table_name**: The name of the table to query.
    - **limit**: Maximum number of rows to return (default: 100).
    - **offset**: Number of rows to skip (default: 0).
    - **filters_string**: A comma-separated string of key=value pairs for filtering (e.g., `category=water,price=100`).
...
@app.get("/logs", summary="Get API activity log")
async def get_api_activity_log():
    """
    Returns the contents of the API activity log file as plain text.
    """
